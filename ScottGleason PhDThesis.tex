...
The production of thin films by \gls{pvd} requires targets of appropriate composition for the desired film stoichiometry. These master alloy targets are produced by induction melting of pure constituent elements.

\subsection{Base Elements}
The master alloy is prepared from pure constituent elements of Mg (99.85 wt\%), Zn (99.995 wt\%), and Ca (99.8 wt\%). Each element is polished and filed to removal surface contamination and oxides.

The total weight of each constituent element required for a master alloy charged is automatically calculated via a developed MS Excel workbook. This tool computes the necessary total weights of each element from the available innovatory, checks the expected master alloy composition, and provides a space for notes entire process (i.e. heating cycles, observations, possible future refinements, etc.).
...
\begin{figure}[htbp]
	...
	%Image 1
	\begin{subfigure}[htbp]{0.49\textwidth}
		\includegraphics[width=\textwidth]{LAWS_Induction_Casting.png}
		\caption{}
		\label{fig:LawsCasting}
	\end{subfigure}
	%Image 2
	\begin{subfigure}[htbp]{0.49\textwidth}
		\includegraphics[width=\textwidth]{Laws_Copper_Mould.png}
		\caption{}
		\label{fig:LawsMould}
	\end{subfigure}
	\caption[(a) Inducting casting setup. (b) Copper plate mould.]{(a) Inducting casting setup. (b) Copper plate mould. Reproduced from \cite{Laws2007}.}%global caption
	\label{fig:MgZnCaPlots}
\end{figure}

\subsection{Final Shaping}
Once a master alloy plate is cast the riser is removed. The plate is mounted in a polymer grip vice and the riser carefully cut off with a 24 or 32 \acrshort{tpi} hacksaw. Paper is placed under the vice grips to capture the dust for later analysis. \todo{add photo riser removal}

The target is extracted from the plate by a $32 mm$ diameter diamond bit drill on a 'xxx brand' drill press at xxx \acrshort{rpm}. The plate is mounted in a polymer grip horizontal vice on top of cut plywood for dampening. Drilling is at a slow rate of approximately $3mm$ per $10$ minutes, with a constant stream of distilled water for lubrication. 
...
\todo{Add photo of drill press set up, with bit, drip tray, etc.}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.75\textwidth]{Cao_SputterMachine.png}
